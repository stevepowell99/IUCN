...
all=read.csv("/home/steve/steve@promente.net/Methods/R-methods/mapAndCountryDataStuff/ISO-3166-Countries-with-Regional-Codes/all/all.csv", stringsAsFactors=F)

WAfCodes=all[,2][all[,7]=="11"]
CAfCodes=all[,2][all[,7]=="17"]
WAfNames=all[,1][all[,7]=="11"]
CAfNames=all[,1][all[,7]=="17"]
...
write.csv(mbz,"MBZ.csv")
saveRDS(mbz,"mbz")
} else mbz=readRDS(file = "mbz")
...
root=xmlRoot(pargot)
rows=xpathSApply(pargot,"//div[@class='filterData']")#,xmlValue)
...
write.csv(t(s),"cepf.csv")
library(plyr)
saveRDS(cepf,"cepf")
} else cepf=readRDS(file = "cepf")
...
colnames(darwin)=xc("ID Ref StartDate Country Pounds Details URL More")
write.csv(darwin,"darwin.csv")
saveRDS(darwin,"darwin")
} else darwin=readRDS(file = "darwin")
...
write.csv(monaco,"monaco.csv")
saveRDS(monaco,"monaco")
} else monaco=readRDS(file = "monaco")
...
```{r joining}
library(plyr)
all=rbind.fill(cepf,mbz,ppi,monaco,darwin)
all$Country=as.character(r$Country)
target=with(all,paste(Country,Details,More,Title,NGO,Report))
